                ...
                <div class="col-md-3">
                    <div class="sidebar">
                      <ul>
						...
						<?php  if($this->session->userdata('admin_session')['user_group_id'] == '3' || $this->session->userdata('admin_session')['user_group_id'] == '1' ){ ?><li><a href="<?php echo base_url(); ?>verifier-manager" class="active" ><b style="padding: 5px 7px;background:rgba(255,255,255,0.4);border-radius:100%;/* width: 23px; */font-size: 13px;"><?php echo $vcount;?></b>&nbsp;Verifier Manager </a></li><?php  } ?>
						<?php  if($this->session->userdata('admin_session')['user_group_id'] == '3' || $this->session->userdata('admin_session')['user_group_id'] == '2'){ ?><li><a href="<?php echo base_url(); ?>approval-manager"><b style="padding: 5px 7px;background:rgba(255,255,255,0.4);border-radius:100%;/* width: 23px; */font-size: 13px;"><?php echo $acount;?></b>&nbsp;Approval Manager </a></li><?php  } ?>
                ...
                <div class="col-md-9">
                    <div class="content content-box pb-3">
                        <div class="card-header m-b-10">
                            <div class="row">
                                <div class="col-md-12">
                                    <h4 class="m-0"> Verifier Manager </h4>
                                </div>
                            </div>
                        </div>
								...
								//print_r($cdoc);
								  ...
								  ?>
									 <tr>
										...
										<td><?php if($cdoc['isverified'] == true && $cdoc['isrejected'] == true){ echo '<b>Rejected By: </b>'.ucfirst($cdoc['vname']); ?><br/><a href="mailto:<?php echo $cdoc['vemail']; ?>"><?php echo  $cdoc['vemail']; ?></a><br/><a href="callto:<?php echo  $cdoc['vmobile']; ?>"><?php echo $cdoc['vmobile']; ?></a><br/><b>Reason: </b><?php echo  $cdoc['remarks'];?><?php  } ?></td>
                                        <td style="word-wrap: break-word;min-width: 126px;max-width: 126px;"><?php  if($cdoc['isverified'] == true){ echo '<b style="color:#FFC107;">Verified</b><br/>'; } if($cdoc['isrejected'] == true ){ echo '<b style="color:red;">Rejected</b>';  }  if($cdoc['isrejected'] == false && $cdoc['isverified'] == false) { ?>  <button class="btn btn-sm btn-warning" style="padding: 5px 10px 5px 10px;color:#fff;" onclick="doverify(<?php echo $cdoc['docid'] ?>,'Verify')">Verify</button>
										<button class="btn btn-sm btn-danger" onclick="doreject(<?php echo $cdoc['docid'] ?>,'Reject')">Reject</button> <?php } ?></td>
